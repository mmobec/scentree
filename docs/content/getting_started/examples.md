...
Scenario fan generation is decoupled from tree construction to provide greater flexibility. Users may already have their own procedure for generating scenario fans, in which case they are not required to use `scentree` for this step. Instead, they can generate the scenario fans externally and use `scentree` solely to construct the corresponding scenario tree.
...
Note that the output is a dictionary. Inspect the returned object to access the generated scenarios. In this example, the `scenarios` entry is a list of length four, where each element contains a set of scenarios.

This step involves the random sampling of the estimated residuals. To ensure reproducibility, the `generate_scenario_fans` method accepts an optional `seed` parameter. Using the same seed, together with identical input data and model configuration, will produce the same scenario fans.
